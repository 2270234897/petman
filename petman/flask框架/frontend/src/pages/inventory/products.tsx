...
export function InventoryProducts() {
  ...
  const handleEdit = (productId: number, productName: string) => {
    // 实际编辑逻辑
    toast.info(`正在编辑商品: ${productName}`)
    // 这里可以添加实际的编辑逻辑，比如打开编辑模态框
  }
                ...
                <div key={item.productID} className="p-4 border rounded-lg hover:bg-accent/50 transition-colors bg-white dark:bg-gray-800">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex space-x-2">
                      <Button variant="default" size="sm" onClick={() => handleEdit(item.productID, item.product_name)} className="btn-vibrant-blue">
