        ...
        with connection.cursor() as cursor:
            ...
            if 'specs' in data and isinstance(data['specs'], list):
                ...
                for i, spec in enumerate(data['specs']):
                    ...
                    spec_sql = """
                    INSERT INTO spec (specID, product_productID, spec_type_id, spec_name, spec_value, barcode, picture)
            ...
            if 'specs' in data and isinstance(data['specs'], list):
                ...
                for i, spec in enumerate(data['specs']):
                    ...
                    if i < len(existing_spec_ids):
                        ...
                        # 更新现有规格
                        spec_sql = """
                        UPDATE spec SET spec_type_id=%s, spec_name=%s, spec_value=%s, barcode=%s, picture=%s
                        WHERE specID=%s
                        """
                        cursor.execute(spec_sql, (
                            spec_type_id,
...
def get_specs():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            sql = """
            SELECT s.specID, s.spec_name, s.spec_value, s.barcode, s.picture,
...
def add_spec():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            sql = """
            INSERT INTO spec (specID, product_productID, spec_name, spec_value, barcode, picture)
            VALUES (%s, %s, %s, %s, %s, %s)
            """
            cursor.execute(sql, (
                new_id,
